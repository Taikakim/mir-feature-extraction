#!/usr/bin/env python3
"""
rename_stems_strip_numbers.py — Strip numeric prefixes from goa-stems track folders.
...
Categories
----------
RENAME    stripped name exists in crops, no conflict in goa-stems → safe to rename
MERGE     stripped name already exists as an unnumbered folder in goa-stems
...
Usage:
    python scripts/rename_stems_strip_numbers.py --dry-run
    python scripts/rename_stems_strip_numbers.py
    python scripts/rename_stems_strip_numbers.py --verbose
"""
...
def main():
    ...
    stems_dirs          = [d for d in GOA_STEMS.iterdir() if d.is_dir()]
    ...
    print(f"goa-stems folders total      : {len(stems_dirs)}")
    ...
    duplicates = []   # (stripped_name, [folders])  — multiple numbered, same stripped
    orphans    = []   # (stripped_name, folder)     — not in crops
    ...
    for stripped, folders in sorted(by_stripped.items()):
        ...
        if is_duplicate:
            ...
            renames.append((folders[0], GOA_STEMS / stripped))
    ...
    renamed = 0
    late_merges = 0  # destination appeared after dry-run (encode was still running)
    for src, dst in renames:
        n = folder_npy_count(src)
        if not dry and dst.exists():
            # Destination appeared since the scan (encode created it mid-run) — merge
    ...
    for src, dst in merges:
        ...
        if not dry:
            for f in files_to_move:
                shutil.move(str(f), str(dst / f.name))
                merged_files += 1
            # Remove numbered folder if now empty
            remaining = list(src.iterdir())
            if not remaining:
                src.rmdir()
            else:

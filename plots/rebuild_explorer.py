#!/usr/bin/env python3
"""Rebuild feature_explorer.html with 7 modes and enhancements."""
import os

SRC = os.path.join(os.path.dirname(__file__), 'feature_explorer.html')
...
b1 = content.find('\nconst DATA =')
b2 = content.find('\nlet mode =')
b3 = content.rindex('</script>')
assert b1 > 0 and b2 > b1 and b3 > b2, "Boundaries not found"

data_blob = content[b1:b2]
...
<style>
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .controls {
    ...
    background: #16213e; border-bottom: 1px solid #333355; align-items: center;
  }
  ...
  select {
    background: #0f3460; color: #e0e0e0; border: 1px solid #333355;
    padding: 5px 8px; border-radius: 4px; font-size: 12px; min-width: 150px; cursor: pointer;
  }
  select:hover { border-color: #e94560; }
  .mode-btn {
    background: #0f3460; color: #ccc; border: 1px solid #333355;
  ...
  .log-btn {
    background: #0f3460; color: #888; border: 1px solid #333355;
    padding: 3px 8px; border-radius: 3px; cursor: pointer; font-size: 11px; font-weight: bold;
  }
  .log-btn.active { background: #e94560; color: white; border-color: #e94560; }
  .log-btn:hover { border-color: #e94560; color: #ccc; }
  .chk-label {
    display: flex; align-items: center; gap: 6px; cursor: pointer;
    font-size: 12px; color: #ccc; white-space: nowrap;
    padding: 5px 8px; border: 1px solid #333355; border-radius: 4px;
    background: #0f3460; transition: border-color 0.15s;
  }
  .chk-label:hover { border-color: #e94560; }
  .chk-label input[type=checkbox] { accent-color: #e94560; width: 13px; height: 13px; cursor: pointer; }
  .info { font-size: 11px; color: #666; padding: 3px 14px; background: #12192e; flex-shrink: 0; }
  .help-box {
    font-size: 11px; color: #aaa; padding: 5px 14px; background: #12192e;
    border-bottom: 1px solid #222244; line-height: 1.6; flex-shrink: 0;
  }
  .help-box .feat { color: #00d2ff; font-weight: 600; }
  .help-box .desc { color: #999; }
  .formula-panel {
    position: fixed; top: 8px; right: 8px; width: 360px;
    background: rgba(15,20,40,0.97); border: 1px solid #333355;
    border-radius: 8px; padding: 12px 14px; z-index: 500;
    font-size: 12px; color: #bbb; max-height: 65vh; overflow-y: auto;
    backdrop-filter: blur(8px); box-shadow: 0 4px 20px rgba(0,0,0,0.6);
  }
  .formula-panel h3 { font-size: 13px; margin: 0 0 10px; color: #e94560; border-bottom: 1px solid #333355; padding-bottom: 6px; }
  .formula-entry { margin-bottom: 12px; padding-bottom: 10px; border-bottom: 1px solid #1a2040; }
  .formula-entry:last-child { border-bottom: none; margin-bottom: 0; padding-bottom: 0; }
  .formula-entry .fe-name { color: #00d2ff; font-weight: 700; font-size: 12px; }
  .formula-entry .fe-src { font-size: 10px; color: #666; margin: 2px 0; }
  .formula-entry .fe-lib { font-size: 11px; color: #e94560; font-weight: 600; margin: 2px 0; }
  .formula-entry pre {
    background: #0c1025; padding: 5px 7px; border-radius: 4px;
    font-family: \'Fira Code\', \'Consolas\', monospace; font-size: 11px;
    color: #8be9fd; line-height: 1.5; margin: 4px 0 0; white-space: pre-wrap;
    border: 1px solid #1a2540;
  }
  .fe-warn {
    display: block; margin-top: 4px; padding: 4px 8px;
    background: rgba(233,69,96,0.12); border-left: 3px solid #e94560;
    font-size: 10px; color: #e94560; line-height: 1.4; border-radius: 0 4px 4px 0;
  }
  /* Lasso select sidebar */
  .sel-panel {
    position: fixed; right: 0; top: 50%; transform: translateY(-50%);
    width: 240px; max-height: 60vh; background: rgba(15,20,40,0.97);
    border: 1px solid #333355; border-right: none; border-radius: 8px 0 0 8px;
    padding: 10px; overflow-y: auto; z-index: 200; display: none;
  }
  .sel-panel h4 { font-size: 11px; color: #e94560; margin-bottom: 6px; padding-bottom: 4px; border-bottom: 1px solid #333355; }
  .track-item { font-size: 11px; color: #ccc; padding: 4px 0; border-bottom: 1px solid #222244; cursor: pointer; }
  .track-item:hover { color: white; }
  /* Similarity NN panel */
  #sim-nn-panel {
    position: fixed; right: 0; top: 50%; transform: translateY(-50%);
    width: 260px; max-height: 75vh; background: rgba(15,20,40,0.97);
    border: 1px solid #333355; border-right: none; border-radius: 8px 0 0 8px;
    padding: 10px; overflow: hidden; z-index: 200; display: none;
    flex-direction: column;
  }
  #nn-feat-row { display: flex; flex-wrap: wrap; gap: 3px; margin-bottom: 6px; }
  #nn-feat-row select { min-width: 110px; font-size: 10px; padding: 3px 5px; }
  #nn-track-list { overflow-y: auto; flex: 1; }
  .nn-row { padding: 4px 2px; border-bottom: 1px solid #1a2040; cursor: pointer; }
  .nn-row:hover { background: rgba(255,255,255,0.04); }
  /* Track detail panel */
  #track-panel {
    position: fixed; bottom: 36px; left: 14px; width: 250px;
    background: rgba(15,20,40,0.97); border: 1px solid #333355;
    border-radius: 8px; padding: 11px; z-index: 300; display: none;
    box-shadow: 0 4px 20px rgba(0,0,0,0.5);
  }
  #tp-name { font-size: 13px; font-weight: bold; color: white; margin-bottom: 6px; line-height: 1.3; }
  #tp-features { font-size: 11px; color: #aaa; line-height: 1.8; margin-bottom: 8px; }
  #tp-links { display: flex; gap: 10px; flex-wrap: wrap; }
  #tp-links a { font-size: 11px; text-decoration: none; }
...
<div class="controls">
  ...
  <!-- Color by (scatter) -->
  <div class="control-group" id="colorby-group" style="display:none">
    <label>Color By</label>
    <select id="sel-colorby" onchange="updatePlot()"></select>
  </div>
  <!-- Quadrant -->
  ...
  <div class="control-group" id="radar-controls" style="display:none">
    <label>Track</label>
    <select id="sel-radar-track" onchange="updatePlot()" style="min-width:200px;"></select>
  </div>
  <!-- Similarity: reference track -->
  <div class="control-group" id="sim-ref-group" style="display:none">
    <label>Reference Track</label>
    <select id="sel-sim-ref" onchange="updatePlot()" style="min-width:200px;"></select>
  </div>
  ...
  <div style="margin-left:auto;display:flex;gap:6px;align-items:center;flex-wrap:wrap;">
    ...
    <div id="outlier-group" style="display:none">
      <label class="chk-label" title="Highlight outliers: |v - mean| > 2&#963;">
        <input type="checkbox" id="chk-outliers" onchange="updatePlot()"> Outliers
      </label>
    </div>
...
<div class="sel-panel" id="sel-panel">
  <h4>Selected Tracks <button onclick="hideSidebar()" style="float:right;background:none;border:none;color:#888;cursor:pointer;font-size:13px;">&#x2715;</button></h4>
  <div id="sel-list"></div>
</div>

<div id="sim-nn-panel" style="display:none;flex-direction:column;">
  <div style="font-size:11px;color:#e94560;font-weight:bold;margin-bottom:6px;padding-bottom:5px;border-bottom:1px solid #333355;">Most Similar</div>
  <div id="nn-feat-row">
    <select id="nn-f1" onchange="updateSimNN()" style="min-width:110px;font-size:10px;padding:3px 5px;"></select>
    <select id="nn-f2" onchange="updateSimNN()" style="min-width:110px;font-size:10px;padding:3px 5px;"></select>
    <select id="nn-f3" onchange="updateSimNN()" style="min-width:110px;font-size:10px;padding:3px 5px;"></select>
    <select id="nn-f4" onchange="updateSimNN()" style="min-width:110px;font-size:10px;padding:3px 5px;"></select>
    <select id="nn-f5" onchange="updateSimNN()" style="min-width:110px;font-size:10px;padding:3px 5px;"></select>
  </div>
...
<div id="track-panel">
  <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:6px;">
    <div id="tp-name"></div>
    <button onclick="hideTrackPanel()" style="background:none;border:none;color:#888;cursor:pointer;font-size:15px;padding:0 0 0 6px;flex-shrink:0;">&#x2715;</button>
  </div>
...
function updateHelp() {
  ...
  if (mode==='scatter') {
    ...
    lines.push('<span class="feat">Radar: '+t+'</span> \u2014 <span class="desc">Normalized values (0\u20131) on curated features. Red = above mean, Blue = below mean.</span>');
...
function populateSelects() {
  ...
  // Color-by (with none option)
  const cbSel = document.getElementById('sel-colorby');
  if (cbSel) {
    const prev = cbSel.value;
    cbSel.innerHTML = '<option value="">&#8212; none &#8212;</option>';
    df.forEach(f => { const o=document.createElement('option'); o.value=f; o.textContent=f; cbSel.appendChild(o); });
    if (prev && df.includes(prev)) cbSel.value = prev;
  }
...
function setMode(m) {
  mode = m;
  ['scatter','quadrant','histogram','radar','heatmap','parallel','similarity'].forEach(n => {
    document.getElementById('btn-'+n).className = 'mode-btn'+(n===m?' active':'');
  });
  const V = {
    'scatter-controls':   m==='scatter',
    'scatter-controls-y': m==='scatter',
    'colorby-group':      m==='scatter',
    'quad-xp':            m==='quadrant',
    'quad-xn':            m==='quadrant',
    'quad-yp':            m==='quadrant',
    'quad-yn':            m==='quadrant',
...
function showTrackPanel(idx, featureKeys) {
  ...
  lh += '<a href="https://tidal.com/search?q='+encodeURIComponent(TRACKS[idx])+'" target="_blank" style="color:#00d2ff;">\uD83C\uDF0A Tidal</a>';
...
function updateScatter() {
  const kx = document.getElementById('sel-x').value;
  const ky = document.getElementById('sel-y').value;
  const kc = document.getElementById('sel-colorby').value;
  const showTrend    = document.getElementById('chk-trend').checked;
  const showOutliers = document.getElementById('chk-outliers').checked;
  if (!kx||!ky) return;
  ...
  const hover = names.map((n,j) =>
    '<b>'+n+'</b><br>'+kx+': '+x[j].toFixed(4)+'<br>'+ky+': '+y[j].toFixed(4));

  let traces;
  if (showOutliers && x.length>0) {
    ...
    const cvals = trackIdxs.map(i => DATA[kc][i]);
    traces=[{ x, y, mode:'markers', type:'scattergl',
      marker:{color:cvals,colorscale:'Viridis',showscale:true,
              colorbar:{title:kc,thickness:14},size:5,opacity:0.7},
  ...
  if (showTrend && x.length>2) {
    ...
    traces.push({x:[xmn,xmx],y:[s*xmn+ic,s*xmx+ic],mode:'lines',
  ...
  Plotly.react('plot',traces,{
    ...
    xaxis:{title:kx+ux,type:logX?'log':'linear'},
    yaxis:{title:ky+uy,type:logY?'log':'linear'},
  ...
  pd.on('plotly_click',function(d){
    if(d.points&&d.points.length>0){
      ...
      if(idx!==undefined) showTrackPanel(idx,[kx,ky]);
    }
  });
  ...
  pd.on('plotly_deselect',hideSidebar);
  document.getElementById('info-bar').textContent=kx+' vs '+ky+' | r = '+r.toFixed(3)+' | '+x.length+' tracks';
}
...
function updateHistogram() {
  ...
  const nbins=Math.min(30,Math.ceil(Math.sqrt(n)));
  ...
  document.getElementById('info-bar').textContent=kh+' | n='+n+' | \u03BC='+m.toFixed(3)+' | \u03C3='+s.toFixed(3);
}

// ---- RADAR -----------------------------------------------------------------
function updateRadar() {
  const tName=document.getElementById('sel-radar-track').value;
  const tIdx=TRACKS.indexOf(tName);
  if(tIdx<0) return;
  const cf=CURATED.filter(f=>FEATURES.includes(f)&&DATA[f][tIdx]!==null&&isFinite(DATA[f][tIdx]));
  if(!cf.length) return;
  const normV=cf.map(f=>{
    const valid=DATA[f].filter(v=>v!==null&&isFinite(v));
    const mn=Math.min(...valid),mx=Math.max(...valid);
    return mx-mn<1e-10?0.5:(DATA[f][tIdx]-mn)/(mx-mn);
  });
  const normM=cf.map(f=>{
    const valid=DATA[f].filter(v=>v!==null&&isFinite(v));
    const mn=Math.min(...valid),mx=Math.max(...valid),m=mean(valid);
    return mx-mn<1e-10?0.5:(m-mn)/(mx-mn);
  });
  const labels=cf.map(shortLabel);
  const barColors=normV.map((v,i)=>v>=normM[i]?'#e94560':'#00d2ff');
  Plotly.react('plot',[
    {type:'barpolar',r:normV,theta:labels,
     marker:{color:barColors,opacity:0.8,line:{color:'rgba(0,0,0,0.3)',width:0.5}},name:tName},
    {type:'barpolar',r:normM,theta:labels,
     marker:{color:'rgba(180,180,180,0.12)',line:{color:'#556',width:1}},name:'dataset mean'}
  ],{
    title:{text:'Feature Radar: '+tName,font:{size:14}},
    polar:{radialaxis:{visible:true,range:[0,1.15],tickfont:{size:9},gridcolor:'#333355'},
           angularaxis:{tickfont:{size:10},gridcolor:'#333355'},bgcolor:'#16213e'},
    template:'plotly_dark',paper_bgcolor:'#1a1a2e',font:{color:'#cccccc'},
    showlegend:true,legend:{x:0.82,y:0.98,bgcolor:'rgba(15,52,96,0.8)'},
    margin:{t:60,b:30,l:30,r:30}
  },{responsive:true});
  document.getElementById('info-bar').textContent='Radar: '+tName+' | '+cf.length+' features';
}
...
function updateSimilarity() {
  ...
  Plotly.react('plot',[
    {x:oX,y:oY,mode:'markers',type:'scattergl',
     marker:{color:oD,colorscale:[[0,'#e94560'],[0.5,'#552244'],[1,'#111133']],
  ...
  document.getElementById('plot').on('plotly_click',function(d){
    if(d.points&&d.points.length>0){
      const pt=d.points[0];
      if(pt.customdata!==undefined) showTrackPanel(pt.customdata,[kxp,kxn,kyp,kyn]);
    }
  });
  document.getElementById('info-bar').textContent='Similarity: '+refName+' | '+idxs.length+' tracks';
  updateSimNN();
}
...
function updateSimNN() {
  ...
  list.innerHTML=top.map((item,rank)=>{
    ...
    nnFeats.forEach((f,fi)=>{
      ...
      const rf=refNorm[fi];
      barsHtml+='<div title="'+f+': '+(v!==null?v.toFixed(2):'N/A')+
        '" style="position:relative;width:22px;height:6px;background:#0f3460;border-radius:1px;overflow:visible;">'+
    ...
    barsHtml+='</div>';
    return '<div class="nn-row" onclick="showTrackPanel('+t+','+JSON.stringify(nnFeats)+')">' +
      '<div style="font-size:10px;color:#ccc;">'+(rank+1)+'. '+TRACKS[t]+'</div>' +
...
# ---------------------------------------------------------------------------
new_content = NEW_HTML + data_blob + NEW_JS + '\n</script>\n</body>\n</html>'

tmp = SRC + '.new'
with open(tmp, 'w', encoding='utf-8') as f:
    f.write(new_content)
os.replace(tmp, SRC)
print(f'Done. {len(new_content):,} chars ({len(new_content)-len(content):+,} vs original)')
